...
const HistoryPage = () => {
  ...
  useEffect(() => {
    ...
    setSearchResults(searchStorage.getSearchHistory())
  }, [])
  ...
  const handleExportSearch = (searchId) => {
    const search = searchResults.find((s) => s.id === searchId)
    if (search) {
  ...
  const handleDeleteSearch = (searchId) => {
    if (searchStorage.deleteSearchById(searchId)) {
      setSearchResults(searchStorage.getSearchHistory())
    }
  }
                ...
                >
                  <CardContent className="p-8">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <Link
                        to={`/results?searchId=${search.id}&businessType=${search.searchParams.businessType}&locations=${search.searchParams.locations.join(",")}&numberOfBusinesses=${search.searchParams.numberOfBusinesses}`}
                      >
